...
- The **"getSitumData" component's code is a mess**. I'm still not familiar with useEffect and dealing with asynchronous external fetching inside React. I think that I should turn it into a regular component that simply updates a state variable when the Promise resolves, but it feels a bit too much since I just call it once in the app, so there's really no state to update after the first time. So maybe it should be just a regular async function call and then trigger a re-render when the promise resolves. But I'm not sure how to do that.
